...
export default class WrappedCardRenderer {
  ...
  private drawBackground(ctx: SKRSContext2D, colorTop: string, colorBottom: string): void {
  ...
  private fillTextTruncated(ctx: SKRSContext2D, text: string, x: number, y: number, maxWidth: number): void {
  ...
  private wrapText(ctx: SKRSContext2D, text: string, x: number, y: number, maxWidth: number, lineHeight: number): void {
